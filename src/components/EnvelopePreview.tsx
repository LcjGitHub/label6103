...
const EnvelopePreview = forwardRef<HTMLDivElement, EnvelopePreviewProps>(function EnvelopePreview(
  { forceZoomPercent },
  ref,
) {
  const { data, layout, size, side, zoomPercent } = useEnvelope();

  const { width: widthPx, height: heightPx } = getEnvelopePixelSize(size.widthMm, size.heightMm, 1);

  const effectiveZoom = forceZoomPercent !== undefined ? forceZoomPercent : zoomPercent;
