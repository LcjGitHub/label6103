...
import { generateTagId, TAG_LIST_KEY, type Tag } from '../types/envelope';
...
export function useTagList(): UseTagListReturn {
  ...
  const updateTag = useCallback((id: string, name: string, color: string) => {
    setTagList((prev) => prev.map((t) => (t.id === id ? { ...t, name: name.trim(), color } : t)));
    setError(null);
  }, []);
  ...
  const isTagNameDuplicate = useCallback(
    (name: string, excludeId?: string) => {
      return tagList.some((t) => t.name.trim() === name.trim() && t.id !== excludeId);
    },
    [tagList],
  );
